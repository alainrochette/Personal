...
class Player(pygame.sprite.Sprite):
	...
	def createBar(self, difficulty, edit):
		...
		if not edit:
			self.edit = False
			diffBars = {"Easy": 100, "Medium": 90, "Hard": 80, "Expert": 70}
			...
			self.center_bar = pygame.Surface([self.menu.W, 4])
			self.center_bar_rect = self.center_bar.get_rect()
			self.center_bar_rect.x = 0
			self.center_bar_rect.y = self.whitebar_rect.y + self.whitebarheight / 2
	...
	def drawSelf(self):
		...
		if not self.edit: pygame.draw.rect(self.screen, self.menu.theme_opp, self.center_bar_rect)
